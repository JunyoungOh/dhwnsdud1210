...
import {
  ...
  CalendarPlus, AlertCircle, Star, StarOff, Menu, Folder,
  FolderPlus, FolderX, ChevronLeft, ChevronRight, ChevronsLeft, ChevronsRight,
  Layers, LineChart, Clock, Sparkles, ExternalLink
} from 'lucide-react';
      ...
      <div className="relative bg-white rounded-xl shadow-2xl w-full max-w-4xl max-h-[85vh] p-6 overflow-hidden">
              ...
              {items.length === 0 ? (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {items.map(({ profile, score }) => (
                    <button
                      key={profile.id}
                      className="text-left border rounded-lg p-4 bg-white shadow-sm hover:shadow-md transition"
        ...
        {!!focus && (
          <div className="relative bg-gray-50 rounded-lg p-4 border">
            <button onClick={() => setFocus(null)} className="absolute top-3 left-3 text-gray-600 hover:text-gray-900 text-sm">← 목록으로</button>
            <div className="pl-20">
              <ProfileCard
                profile={focus}
                onUpdate={onUpdate}
...
const StarFolderModal = ({ open, onClose, folders, initial, onSave, onCreateFolder }) => {
  const [selected, setSelected] = useState(initial && initial.length ? initial : ['전체']);
  const [newName, setNewName] = useState('');
  useEffect(() => { setSelected(initial && initial.length ? initial : ['전체']); }, [open, initial]);
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative bg-white rounded-xl shadow-2xl w-full max-w-md p-6">
        ...
        <div className="space-y-2 max-h-[50vh] overflow-y-auto pr-2">
          {folders.map(f => (
            <label key={f} className="flex items-center gap-2 text-sm">
              ...
              <span className="flex items-center gap-2"><Folder size={16} className="text-gray-500" />{f}</span>
            </label>
          ))}
        </div>
        ...
        <div className="mt-4 flex gap-2">
          ...
          />
          <button
            onClick={async () => { if (!newName.trim()) return;
              await onCreateFolder(newName.trim());
              setNewName('');
            }}
            className="px-3 py-1 rounded-md bg-gray-100 text-sm hover:bg-gray-200 flex items-center gap-1"
          >
        ...
        <div className="mt-5 flex justify-end gap-2">
          <button onClick={onClose} className="text-sm px-3 py-1 rounded-md bg-gray-100">취소</button>
          <button onClick={save} className="text-sm px-3 py-1 rounded-md bg-yellow-400 text-white">저장</button>
        </div>
      </div>
    </div>
  );
...
const ProfileCard = ({
  ...
  accessCode, onSyncOne, onShowSimilar, onToggleStar, dense = false
}) => {
  ...
  return (
    <div className={`bg-white rounded-xl shadow border p-4 relative ${dense ? '' : 'flex flex-col md:flex-row gap-3'}`}>
      ...
      <StarFolderModal
        ...
        folders={profile._folderUniverse || ['전체']}
        initial={profile.starredFolders || (profile.starred ? ['전체'] : [])}
        onSave={(selected) => onToggleStar(profile.id, selected)}
        onCreateFolder={profile._onCreateFolder || (()=>{})}
      />
    </div>
  );
};
    ...
    <div className="space-y-4">
        ...
        <div className="flex flex-wrap items-center gap-2">
          {folders.map(f => (
            <button key={f}
              ...
              className={`px-3 py-1 rounded-full border text-sm flex items-center gap-2 ${selectedFolder===f?'bg-yellow-400 text-white border-yellow-400':'bg-white hover:bg-gray-50'}`}>
              <Folder size={14} /> {f}
            </button>
          ))}
        </div>
        <div className="flex items-center gap-2">
          {!delMode ? (
            ...
            <>
              ...
              <button onClick={async () => {
                const targets = delSet.filter(f => f !== '전체');
                if (!targets.length) { setDelMode(false); return; }
                if (!window.confirm(`다음 폴더를 삭제할까요?\n${targets.join(', ')}`)) return;
                await onDeleteFolders(targets);
                setDelMode(false);
              }} className="px-3 py-1 rounded-md bg-red-50 text-red-600 hover:bg-red-100 text-sm">삭제 확정</button>
              <button onClick={()=>setDelMode(false)} className="px-3 py-1 rounded-md bg-gray-100 hover:bg-gray-200 text-sm">취소</button>
            </>
          )}
        </div>
      </div>
      ...
      {delMode && (
        <div className="p-3 border rounded-lg bg-white">
          <div className="text-sm font-semibold mb-2">삭제 대상 선택</div>
          <div className="flex flex-wrap gap-2">
            {folders.map(f => (
              f === '전체' ? (
                <span key={f} className="px-3 py-1 rounded-full border text-xs text-gray-400 flex items-center gap-2"><Folder size={12}/> 전체</span>
              ) : (
                <button key={f}
                  onClick={() => setDelSet(s => s.includes(f) ? s.filter(x=>x!==f) : [...s, f])}
                  className={`px-3 py-1 rounded-full border text-xs flex items-center gap-2 ${delSet.includes(f)?'bg-red-50 text-red-600 border-red-200':'bg-white hover:bg-gray-50'}`}>
                  <Folder size={12}/> {f}
                </button>
              )
            ))}
...
const FunctionsPage = ({ activeSub, setActiveSub, profiles, onUpdate, onDelete, accessCode, onSyncOne, onShowSimilar, onToggleStar }) => {
  ...
  return (
    <div className="space-y-8">
      <div className="flex items-center gap-2">
        <button onClick={()=>setActiveSub('rec')}
          className={`px-3 py-1.5 rounded-md text-sm flex items-center gap-1 ${activeSub==='rec'?'bg-yellow-400 text-white':'bg-white border hover:bg-gray-50'}`}>
          <Sparkles size={16}/> 추천
        </button>
        <button onClick={()=>setActiveSub('long')}
          className={`px-3 py-1.5 rounded-md text-sm flex items-center gap-1 ${activeSub==='long'?'bg-yellow-400 text-white':'bg-white border hover:bg-gray-50'}`}>
          <Clock size={16}/> 장기관리
        </button>
        <button onClick={()=>setActiveSub('graphs')}
          className={`px-3 py-1.5 rounded-md text-sm flex items-center gap-1 ${activeSub==='graphs'?'bg-yellow-400 text-white':'bg-white border hover:bg-gray-50'}`}>
...
const ManagePage = ({ profiles, onUpdate, onDelete, handleFormSubmit, handleBulkAdd, formState, setFormState, accessCode, onSyncOne, onShowSimilar, onToggleStar }) => {
      ...
      <section>
        ...
        <nav className="mt-6 flex items-center justify-center gap-1">
          <button onClick={()=>setCurrentPage(1)} disabled={currentPage===1}
            className="p-2 rounded-md border bg-white hover:bg-gray-50 disabled:opacity-40"><ChevronsLeft size={16}/></button>
          <button onClick={()=>setCurrentPage(p=>Math.max(1,p-1))} disabled={currentPage===1}
            className="p-2 rounded-md border bg-white hover:bg-gray-50 disabled:opacity-40"><ChevronLeft size={16}/></button>
          ...
          <button onClick={()=>setCurrentPage(p=>Math.min(totalPages,p+1))} disabled={currentPage===totalPages}
            className="p-2 rounded-md border bg-white hover:bg-gray-50 disabled:opacity-40"><ChevronRight size={16}/></button>
          <button onClick={()=>setCurrentPage(totalPages)} disabled={currentPage===totalPages}
            className="p-2 rounded-md border bg-white hover:bg-gray-50 disabled:opacity-40"><ChevronsRight size={16}/></button>
        </nav>
      </section>
    </div>
  );
...
export default function App() {
  ...
  useEffect(() => {
    if (!accessCode) return;
    const ref = metaDocRef(db, appId, accessCode);
    (async () => {
      try {
        ...
        console.warn('meta doc load error', e);
        setStarredFolders(['전체']);
        setStarFolderSelected('전체');
      }
    })();
  }, [accessCode, starFolderSelected]);

  const addFolder = async (name) => {
    if (!name || !name.trim()) return;
    const unique = Array.from(new Set([...starredFolders, name.trim()]));
    await setDoc(metaDocRef(db, appId, accessCode), { folders: unique }, { merge: true });
    setStarredFolders(unique);
    setStarFolderSelected(name.trim());
  };
  const deleteFolders = async (targets) => {
    const remain = starredFolders.filter(f => !targets.includes(f));
    if (!remain.includes('전체')) remain.unshift('전체');
    await setDoc(metaDocRef(db, appId, accessCode), { folders: remain }, { merge: true });
    setStarredFolders(remain);
    if (!remain.includes(starFolderSelected)) setStarFolderSelected('전체');
  };
  ...
  const handleFormSubmit = async (e) => {
    ...
    const profileData = {
      ...
      otherInfo: newOtherInfo || null, eventDate,
      expertise: newExpertise || null, priority: newPriority || null,
  ...
  const handleToggleStar = async (profileId, selectedFolders) => {
    if (!activeColRef) return;
    const starred = selectedFolders && selectedFolders.length > 0;
    await updateDoc(doc(activeColRef, profileId), {
      starred, starredFolders: starred ? selectedFolders : []
    });
  };
    ...
    switch (activeMain) {
      ...
      case 'starred': return (
        <StarredPage
          ...
          selectedFolder={starFolderSelected}
          onSelectFolder={setStarFolderSelected}
      ...
      case 'manage': default: return (
        <ManagePage
          profiles={profilesWithFolderHelpers}
          onUpdate={handleUpdate} onDelete={handleDeleteRequest}
          handleFormSubmit={handleFormSubmit} handleBulkAdd={handleBulkAdd}
          formState={{ newName, newCareer, newAge, newOtherInfo, newExpertise, newPriority, newMeetingRecord }}
          setFormState={{ setNewName, setNewCareer, setNewAge, setNewOtherInfo, setNewExpertise, setNewPriority, setNewMeetingRecord }}
          accessCode={accessCode} onSyncOne={handleSyncOneToCalendar}
          onShowSimilar={openSimilarModal} onToggleStar={handleToggleStar}
        />
      );
      ...
      <div className="flex">
        <aside className={`fixed md:static top-[116px] z-30 md:z-auto left-0 h-[calc(100vh-116px)] md:h-auto w-64 bg-white border-r transform transition-transform ${sidebarOpen ? 'translate-x-0' : '-translate-x-full md:translate-x-0'}`}>
          <nav className="p-3 space-y-1 overflow-y-auto h-full">
            ...
            <button onClick={()=>setActiveMain('search')}
            ...
            <button onClick={()=>setActiveMain('starred')}
