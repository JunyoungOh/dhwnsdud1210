...
import { Users, LogOut, Search, Calendar, Zap, UserPlus, KeyRound, Loader2, Edit, Trash2, X as XIcon } from 'lucide-react';
                ...
                <form className="mt-8 space-y-6" onSubmit={handleSubmit}>
                    <div className="relative">
                        ...
                        <input 
                            ...
                            className="w-full pl-10 pr-3 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-yellow-400" 
                            value={codeInput}
...
export default function App() {
  ...
  const handleEdit = (profile) => {
      ...
      setActiveTab(TAB_PAGE.MANAGE); // 수정 시 관리 탭으로 이동
  };
  ...
  const { todayProfiles, upcomingProfiles, searchedProfiles, allOtherProfiles } = useMemo(() => {
    const now = new Date();
    ...
    const searchRes = term ? profiles.filter(p =>
    ...
    return {
        ...
        searchedProfiles: searchRes,
        allOtherProfiles: others.sort((a,b) => a.name.localeCompare(b.name))
    };
  }, [searchTerm, profiles]);
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {todayProfiles.map(profile => (
                    <div key={profile.id} className="bg-white p-4 rounded-lg shadow border-l-4 border-red-400 relative">
                      ...
                      <div className="absolute top-2 right-2 space-x-2"><button onClick={() => handleEdit(profile)} className="text-blue-500 hover:underline text-xs">수정</button><button onClick={() => handleDelete(profile.id)} className="text-red-500 hover:underline text-xs">삭제</button></div>
                    </div>
                  ))}
                </div>
              </section>
